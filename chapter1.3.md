...
在关系数据库系统一统江湖之后，应用软件的生态变得越来越完善和成熟；针对关系数据库的访问和运维工具越来越丰富；SQL也成为了数据访问的接口标准；这些都进一步巩固了关系数据库系统的地位，但同时也限制了数据管理系统向多样化方向发展。80年代和90年代，科技人员陆续提出了若干数据管理系统的变革路径，比如演绎数据库（Deductive Database）和面向对象的数据库（Object Oriented Database），但这些系统都没有成为主流的产品。部分原因是受软件生态固化的影响。与此同时，各种新的功能被不断添加到关系数据库系统中，将关系数据库变得越来越庞大，复杂度到了无以复加的地步。
