...
作为人之常情，我们希望一个系统是完美的 - 它既有强大的功能，又有极致的性能，还非常易用。就功能性、性能和易用性三个笼统的性质而言，我们就已经碰到了难以调和的矛盾。如果一个系统功能强大，它的构造就会相对复杂，而复杂的构造势必带来性能损失。同样，一个功能强大的系统需要提供足够丰富的接口，让用户能够使用这些功能。接口的增加势必降低系统的易用性。类似这样的矛盾很多，我们在后面介绍具体系统时还会碰到不少。这些矛盾表明完美的系统是很难实现的。设计师必须取舍，找到一个合理的折衷点。

正因为折衷，现实中才出现了大量各式各样的数据管理系统。这些系统因为选择了不同的折衷点而表现出不同的优势和弊端，因而也获得了各自的生存空间。然而，系统的多样性对软件开发而言并非好事。一方面，一个软件工程师不可能有精力去学习和掌握太多种类的系统。另一方面，一个软件生态也难以容纳过多的系统。站在软件开发者的角度，对同一类工作，最好只使用一个通用系统，比如，世界上最好只有一个数据管理系统。这种“one size fits all”的想法很实用，也成为不少系统开发者的目标。但是，折衷给“one size fits all”的理想带来了巨大的障碍。在数据管理系统的发展历程中，我们看到“one size fits all”和“one size fits a bunch”此消彼长，一直在抗衡。

如今，曾经被认为是“one size fits all”的关系数据库正面临各种NoSQL和NewSQl数据库的挑战。多样化的趋势在增强。正因为如此，本书选择多种数据管理系统作为讲解对象，避免局限于某一类系统。
